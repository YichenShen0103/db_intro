...
function Dashboard() {
  ...
  const dispatchEmails = async (project = activeProject) => {
    if (!project) return
    try {
  ...
  const refreshStatus = async () => {
    ...
    try {
      // First fetch latest emails
      await projectsAPI.fetchEmails(activeProject.id)
      // Then get updated tracking info
      ...
      <aside className="w-64 bg-white shadow-md flex flex-col">
        ...
        <nav className="flex-1 p-4 space-y-2">
          <button
            ...
            className={`w-full text-left block px-4 py-2 rounded hover:bg-gray-100 transition ${currentView === 'projects' ? 'bg-blue-50 text-blue-600' : ''
              }`}
          >
          ...
          <button
            ...
            className={`w-full text-left block px-4 py-2 rounded hover:bg-gray-100 transition ${currentView === 'teachers' ? 'bg-blue-50 text-blue-600' : ''
              }`}
          >
